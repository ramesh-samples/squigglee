...
public class MasterDataHandlerImpl extends SchemaHandlerImpl implements IMasterDataHandler {
	private static Logger logger = Logger.getLogger("com.squigglee.storage.vdb.MasterDataHandlerImpl");
	...
	public int[] getDataStatus(long id, String dataType) throws TimeSeriesException {
		...
		MVMap<Object, Object> table = getMap(s, dataType);
		
		//DB db = getDatabase("" + id);
		//ConcurrentNavigableMap<Object,Object> table = getTable(db, dataType);
		
		int lastoffset = -1;
		//if (table != null && !table.descendingKeySet().isEmpty())
		//	lastoffset = Integer.parseInt(table.descendingKeySet().first().toString());
		if (table.lastKey() != null)
			lastoffset = (int) Long.parseLong(table.lastKey().toString());
		int firstoffset = -1;
		if (table.firstKey() != null)
			firstoffset = (int) Long.parseLong(table.firstKey().toString());
