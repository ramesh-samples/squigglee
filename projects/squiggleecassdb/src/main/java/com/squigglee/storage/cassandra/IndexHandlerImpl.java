...
public class IndexHandlerImpl extends DataHandlerImpl implements IIndexHandler {
	...
	private static Logger logger = Logger.getLogger("com.squigglee.storage.cassandra.IndexHandlerImpl");
	...
	public byte[] loadSerializedIndex(long id, String idxTableName) throws TimeSeriesException {
		...
		try {
			...
			serialized = indexService.loadSerializedIndex(this.clusterName, this.ln, id, idxTableName);
			//indexService.close();
	...
	private int deleteIndexRecords(List<MasterData> list, String index, IIndex service) throws TimeSeriesException {
		...
		for (MasterData md : list) {
			service.deleteSerializedIndex(this.clusterName, this.ln, md.getId(), index + "_" + md.getId());
		}
		return list.size();
	}
